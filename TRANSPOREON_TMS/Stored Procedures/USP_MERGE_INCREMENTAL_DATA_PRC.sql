...
CREATE PROCEDURE [TRANSPOREON_TMS].[USP_MERGE_INCREMENTAL_DATA_PRC]
...
MERGE TRANSPOREON_TMS.BOOKING_FACT AS TGT
USING
TRANSPOREON_TMS.BOOKING_FACT_STG AS STG
ON
TGT.[TRANSPORT_NO] = STG.[TRANSPORT_NO]
WHEN MATCHED AND 
...
BEGIN
MERGE TRANSPOREON_TMS.CARRIER_SURCHARGE_FACT AS TGT
USING
...
ISNULL(TGT.[SURCHARGE_STATUS_ID],0) 		<>	ISNULL(STG.[SURCHARGE_STATUS_ID],0)
THEN
UPDATE SET 							
	 TGT.[TRANSPORT_NO] 						=		STG.[TRANSPORT_NO]										
	...
	,TGT.[SURCHARGE_PRICE] 						=		TRIM(REPLACE(STG.[SURCHARGE_PRICE],',',''))				
...
,TRIM(REPLACE(STG.[SURCHARGE_PRICE],',','')) 				
...
MERGE TRANSPOREON_TMS.DELIVERY_DOCUMENT_FACT AS TGT
USING
...
TGT.[STATUS] 						<> 			STG.[STATUS] OR 
TGT.[TIMESTAMP]   					<>			STG.[TIMESTAMP] OR
ISNULL(TGT.[INCOTERM_ID],0) 		<>			ISNULL(STG.[INCOTERM_ID],0) OR
ISNULL(TGT.[SHIP_TO_ID],0) 			<>			ISNULL(STG.[SHIP_TO_ID],0) OR
ISNULL(TGT.[SHIPMENT_STATUS_ID],0) 	<>			ISNULL(STG.[SHIPMENT_STATUS_ID],0) OR					
ISNULL(TGT.[SOLD_TO_ID],0) 			<>			ISNULL(STG.[SOLD_TO_ID],0) OR
ISNULL(TGT.[ROUTE_ID],0) 			<>			ISNULL(STG.[ROUTE_ID],0) 
THEN
UPDATE SET 									
 TGT.[TRANSPORT_NO] 								=			STG.TRANSPORT_NO							
...
MERGE TRANSPOREON_TMS.DELIVERY_ITEM_FACT AS TGT
USING
...
MERGE TRANSPOREON_TMS.TRANSPORT_FACT AS TGT
USING
...
TGT.[STATUS] 						<> 		STG.[STATUS] OR 
TGT.[TIMESTAMP]    					<>		STG.[TIMESTAMP] OR
ISNULL(TGT.[SHIPMENT_STATUS_ID],0) 	<>		ISNULL(STG.[SHIPMENT_STATUS_ID],0)
THEN UPDATE SET	
...
MERGE TRANSPOREON_TMS.TRANSPORT_VISIBILITY_FACT AS TGT
USING
TRANSPOREON_TMS.TRANSPORT_VISIBILITY_FACT_STG AS STG
ON
TGT.[DELIVERY_NO]  								= 			STG.[DELIVERY_NO]
AND TGT.TRANSPORT_NO 							=			STG.TRANSPORT_NO
